...
(function attachTtrpgI18n(root){
  ...
  const sourceOrigins=new Set(['user-source','reference-source','language-choice']);
  ...
  const englishReleasedAttackNote=/^(No combat result was published, and the exact attack was released for editing\. You can search for or call out )([^,\r\n]{1,48})(, attack an enemy already confirmed in the scene, or submit a revised action\.)$/u;
  function splitEnglishReleasedAttackPlayerTarget(value){
    const match=String(value==null?'':value).match(englishReleasedAttackNote); if(!match)return null;
    const target=match[2];
    if(target!==target.trim()||!thaiScript.test(target))return null;
    return Object.freeze({prefix:match[1],target:target,suffix:match[3]});
  }
  ...
  function auditEnglishDom(scope,opts){
    ...
    if(walker){
      let n; while((n=walker.nextNode())){
        ...
        if(repair){const replacement=safeEnglishSystemText(n.nodeValue,'interface text');n.nodeValue=replacement;if(replacement===wallReplacement('interface text'))unresolvedOwners.set(p,verdict);}
      }
    }
    const elements=[];
    ...
    elements.forEach(function(el){
      ...
      attrs.forEach(function(attr){
        ...
        if(repair){const replacement=safeEnglishSystemText(value,attr);el.setAttribute(attr,replacement);if(replacement===wallReplacement(attr))unresolvedOwners.set(el,verdict);}
      });
    });
    if(repair){
      elements.forEach(function(el){
        const unresolved=unresolvedOwners.get(el);
        if(unresolved){el.setAttribute('data-language-wall',unresolved);return;}
        if(!el.hasAttribute||!el.hasAttribute('data-language-wall'))return;
        const directDash=Array.from(el.childNodes||[]).some(function(n){return n.nodeType===3&&String(n.nodeValue||'').trim()===wallFallbacks['interface text'];});
        const fallbackAttr=['placeholder','title','aria-label','value'].some(function(attr){return el.hasAttribute(attr)&&el.getAttribute(attr)===wallFallbacks[attr];});
        if(!directDash&&!fallbackAttr)el.removeAttribute('data-language-wall');
      });
    }
    return findings;
  }
  let wallObserver=null, wallProvider=null;
  // v720: the wall must die with the world that owns it — a live observer whose provider reads a
  // stale EN campaign neutralizes Thai lobby/create screens to em-dashes on every mutation.
  function uninstallDisplayWall(){
    if(wallObserver){try{wallObserver.disconnect();}catch(_e){}}
    wallObserver=null; wallProvider=null;
  }
  ...
  root.TTRPG_I18N=Object.freeze({build:'20260813a1',catalogs:Object.freeze(catalogs),text,errorText,language,latinPublicTextVerdict,catalogRowVisibleForWorld,localizeCatalogRow,displayTextVerdict,safeEnglishSystemText,splitEnglishReleasedAttackPlayerTarget,stripLanguageRepairMetaText,auditEnglishDom,installDisplayWall,uninstallDisplayWall,installDialogWall,translateDom});
})(globalThis);
